...
void main() {
  ...
  testWidgets('unauthenticated users cannot navigate to protected screens', (
    ...
    // "가족이 대신 만들어 드리기"는 계정을 만드는 길이다. 약 등록 화면을
    // 거치지 않고 바로 가입으로 간다 — 처방전 화면은 아예 닿지 않는다.
  ...
  testWidgets('약 등록 화면에 닿아도 로그인 전에는 처방전을 못 찍는다', (tester) async {
    // 로그인 화면에서 이 화면으로 가던 길은 없앴지만, 화면 자체의 가드는
    // 남겨 둔다 — 알림이나 딥링크로 여기 바로 떨어지는 길이 있다.
    // 앱 라우터의 리다이렉트를 빼고 화면이 스스로 막는지만 본다.
    final router = GoRouter(
      ...
      routes: [
        ...
        GoRoute(
          path: '/prescription',
          builder: (_, _) => const PrescriptionScreen(),
        ),
      ],
    );
