...
void main() {
  final complete = <String, dynamic>{'analysis_complete': true,
    'assessment_status': 'SAFE', 'has_risk': false, 'matches': []};
  test('DUR zero, risk, incomplete, nullable and malformed stay distinct', () {
    expect(registrationDurComplete(complete), isTrue);
    expect(registrationDurComplete({...complete, 'assessment_status': 'RISK_FOUND',
      'has_risk': true, 'matches': [{'type':'병용금기'}]}), isTrue);
    for (final raw in [null, {}, {'matches': []}, {...complete,'matches':null},
      {...complete,'matches':[null]}, {...complete,'analysis_complete':false},
      {...complete,'has_risk':null}, {...complete,'incomplete':true}]) {
  ...
  test('refresh failure opt-in throws; old callers keep state, no POST', () async {
    var posts = 0;
    final client = MockClient((r) async {
      if (r.method == 'POST') posts++;
      return http.Response('{"detail":"synthetic"}', 500,
        headers: {'content-type':'application/json'});
    });
    final container = ProviderContainer(overrides:[medicationProvider.overrideWith(
    ...
    await expectLater(controller.refreshFromServer(throwOnError:true), throwsA(isA<ApiException>()));
    expect(container.read(medicationProvider), same(before));
    expect(posts, 0);
  });

  for (final failedPath in ['today-medicines', '/medicines']) {
    testWidgets('manual registration stays saved when $failedPath refresh fails', (tester) async {
      ...
      final client = MockClient((request) async {
        ...
        if (request.method == 'POST') {
          ...
          body = {'registered':true,'prescription_id':'synthetic-prescription',
            'items':[],'dur_result':{...complete,'analysis_complete':false,'has_risk':null}};
        } else if (request.url.path.endsWith('/lookup')) {
          body = {'items':[{'medicine_code':'synthetic-code','display_name':'합성 제품'}]};
        } else if (request.url.path.endsWith(failedPath)) {
          status = 500; body = {'detail':'synthetic failure'};
      ...
      await http.runWithClient(() async {
        await tester.pumpWidget(ProviderScope(child: MaterialApp(home:
          ManualMedicineScreen(onSaved:(value) => saved=value))));
